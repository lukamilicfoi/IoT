...
<?php
	...
	pg_free_result(pgquery("INSERT INTO configuration(username, forward_messages,
			...
			SELECT $sql_username, forward_messages, use_internet_switch_algorithm, nsecs_id,
			nsecs_src, trust_everyone, default_gateway FROM configuration
			WHERE username = 'root';"));
	echo "User $html_username inserted.<br/>\n";
	...
	if ($_SESSION['is_administrator'] && !isset($_POST['is_administrator'])
		...
		pg_free_result(pgquery(substr($query, 0, -2) . ") WHERE username = $sql_username;"));
		echo "User $html_username updated.<br/>\n";
	}
	pg_free_result($result);
} else if (isset($_GET['delete']) && isset($_GET['key'])) {
	$result = pgquery("SELECT TRUE FROM users WHERE username = $sql_key AND NOT is_administrator;");
	if ($_SESSION['is_administrator'] && pg_fetch_row($result) || $_SESSION['is_root']) {
		if (isset($_GET['confirm'])) {
			pg_free_result(pgquery("DELETE FROM users WHERE username = $sql_key;"));
			echo "User $html_key deleted.<br/>\n";
		} else {
?>
			Are you sure?
<?php
			echo "<a href=\"?key=$url_key&amp;delete&amp;confirm\">Yes</a>\n";
?>
			<a href="?">No</a>
<?php
			exit(0);
	...
	pg_free_result(pgquery('UPDATE users SET password = \'' . password_hash($_POST['password'],
			PASSWORD_DEFAULT) . "' WHERE username = {$_SESSION['sql_username']};"));
	echo "Password updated - for username $html_username.<br/>\n";
}
...
<?php
	...
	$result2 = pgquery("SELECT username, TRUE, is_administrator, can_view_tables,
			...
			AND username <> {$_SESSION['sql_username']} ORDER BY username ASC;");
	echo "Viewing table &quot;users&quot; for username {$_SESSION['html_username']} and non-administrators.\n";
} else {
	echo "Viewing table &quot;users&quot; for username {$_SESSION['html_username']}.\n";
}
?>
<table border="1">
	<tbody>
