...
<?php
		...
		$role = pgescaperole1($_POST['username']);
		...
		if (($_SESSION['is_administrator'] && !isset($_POST['is_administrator'])
			...
			pgquery("CREATE ROLE $role;");
			pgquery("GRANT CREATE ON SCHEMA public TO $role;");
			echo "User $h_username inserted.<br/>\n";
		} elseif (!vacuous($_POST['key'])) {
			$s_key = pg_escape_literal($_POST['key']);
			$kole = pgescaperole1($_POST['username']);
			$h_key = htmlspecialchars($_POST['key']);
			if ($_POST['key'] == 'public' || ($_SESSION['is_administrator']
					&& !isset($_POST['is_administrator']) && !is_administrator($s_username)
					|| $_SESSION['is_root']) && isset($_POST['update1']) && ($_POST['key'] != 'root'
				...
				if ($kole != $role) {
					pgquery("ALTER ROLE $kole RENAME TO $role;");
				}
		...
		if (($_SESSION['is_administrator'] && !is_administrator($s_key) || $_SESSION['is_root'])
			...
			if (isset($_GET['confirm'])) {
				...
				pgquery("DROP OWNED BY $kole CASCADE;");
				pgquery("DELETE FROM table_owner WHERE username = $s_key;");
				pgquery("DROP ROLE $kole;");
...
if ($can_edit_yourself && isset($_POST['update2']) && !vacuous($_POST['username'])) {
	...
	pgquery('UPDATE users SET (username' . (!vacuous($_POST['password']) ? ', password' : '')
			...
			? ', \'' . password_hash($_POST['password'], PASSWORD_DEFAULT) . '\'' : '')
			. ") WHERE username = {$_SESSION['s_username']};");
	if ($_SESSION['s1role'] != $role) {
		pgquery("ALTER ROLE {$_SESSION['s1role']} RENAME TO $role;");
	}
