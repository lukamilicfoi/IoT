...
if (checkAuthorization(3, 'view tables')) {
	if (!empty($_GET['add'])) {
		$add = pg_escape_literal($_GET['add']);
		pg_free_result(pgquery('CREATE TABLE ' . pg_escape_identifier($_GET['add'])
				. '(t TIMESTAMP(4) WITHOUT TIME ZONE;'));
		pg_free_result(pgquery("INSERT INTO tables(tablename) VALUES($add);"));
		if (substr($_GET['add'], 0, 1) != 't' || strlen($_GET['add']) != 17) {
			pg_free_result(pgquery("INSERT INTO table_user(tablename, username)
					VALUES($add, NULL);"));
		}
	} else if (!empty($_GET['remove'])) {
		$remove = pg_escape_literal($_GET['add']);
		$result1 = pgquery("SELECT username FROM table_user WHERE tablename = $remove;");
		$result2 = pgquery("SELECT TRUE FROM table_user WHERE tablename = $remove
				AND username = {$_SESSION['sql_username']};");
		$result3 = pgquery("SELECT TRUE FROM table_user INNER JOIN users
				ON table_user.username = users.username WHERE table_user.tablename = $remove
		...
		if (!$row || $row[0] === null || pg_fetch_row($result2) || pg_fetch_row($result3)
			...
			if (isset($_GET['confirm'])) {
				pg_free_result(pgquery('DROP TABLE ' . pg_escape_identifier($_GET['remove'])
						. ';'));
				pg_free_result(pgquery("DELETE FROM tables WHERE tablename = $remove;"));
			} else {
?>
				Are you sure?
<?php
...
<?php
		...
		$result = pgquery("(SELECT pg_class.relname FROM pg_class LEFT OUTER JOIN table_user
				...
				= {$_SESSION['sql_username']} OR NOT users.is_administrator)
		...
		echo "View table (public, &apos;{$_SESSION['html_username']}&apos;&apos;s,
		...
		$result = pgquery("(SELECT pg_class.relname FROM pg_class LEFT OUTER JOIN table_user
				...
				LIKE 't________________' AND pg_class.relname <> 'table_constraints'
				AND (table_user.username IS NULL OR table_user.username
				= {$_SESSION['sql_username']}) ORDER BY pg_class.relname ASC) UNION ALL
				(SELECT tablename FROM table_user WHERE (tablename NOT LIKE 't________________'
				OR tablename = 'table_constraints') AND (username IS NULL
				OR username = {$_SESSION['sql_username']}) ORDER BY tablename ASC);");
		echo "View table (public, &apos;{$_SESSION['username']}&apos;&apos;s shown):\n";
	}
	for ($row = pg_fetch_row($result); $row; $row = pg_fetch_row($result)) {
		echo '<a href="view_table.php?tablename=', urlencode($row[0]), '">',
...
if (checkAuthorization(4, 'send messages')) {
	if (!empty($_GET['msgtosend']) && !empty($_GET['proto_id']) && !empty($_GET['imm_DST'])) {
		pg_free_result(pgquery('CALL send_inject(TRUE, ' . pgescapebytea($_GET['msgtosend']) . ', '
				. pg_escape_literal($_GET['proto_id']) . ', ' . pgescapebytea($_GET['imm_DST'])
...
if (checkAuthorization(5, 'inject messages')) {
	if (!empty($_GET['msgtoinject']) && !empty($_GET['proto_id']) && !empty($_GET['imm_SRC'])) {
		pg_free_result(pgquery('CALL send_inject(FALSE, ' . pgescapebytea($_GET['msgtoinject'])
				. ', ' . pg_escape_literal($_GET['proto_id']) . ', '
				. pg_escape_literal($_GET['imm_SRC']) . ', ' . pgescapebool($_GET['CCF']) . ', '
	...
	if (!empty($_GET['query'])) {
		if (!$_SESSION['is_root']) {
			...
			pg_free_result(pgquery("UPDATE current_username
					SET current_username = {$_SESSION['sql_username']};"));
			pg_close();
		...
		echo 'Query X&apos;', htmlspecialchars($_GET['query']),
				'&apos; sent to database (PostgreSQL ', pg_version()['client'], ").\n";
?>
		<table border="1">
			<tbody>
				<tr>
<?php
					for ($i = 0, $j = pg_num_fields($result); $i < $j; $i++) {
						echo '<th>', pg_field_name($result, $i), ' (', pg_field_type_oid($result, $i),
								")</th>\n";
					}
?>
				</tr>
...
if (checkAuthorization(11, 'manually execute timed rules')) {
	if (!empty($_GET['username']) && !empty($_GET['id'])) {
		...
		if ($_GET['username'] == $_SESSION['username'] || $_SESSION['is_administrator']
			...
			pg_free_result(pgquery("CALL manually_execute_timed_rule($username, "
					. intval($_GET['id']) . ';'));
		}
		pg_free_result($result);
		echo 'For username &apos;', htmlspecialchars($_GET['username']), '&apos; timed rule ',
				htmlspecialchars($_GET['id']), " manually executed.\n";
	}
?>
	<form action="" method="GET">
		For username
