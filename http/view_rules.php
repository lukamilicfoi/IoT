...
<?php
		...
		$sql_username = pg_escape_literal($_GET['username']);
		$id = intval($_GET['id']);
		$html_username = '&apos;' . htmlspecialchars($_GET['username']) . '&apos;';
		if (isset($_GET['insert'])) {
			$result = pgquery("SELECT TRUE FROM users WHERE username = $sql_username
			...
			if ($_GET['username'] == $_SESSION['username'] || $_SESSION['is_administrator']
				...
				pg_free_result(pgquery("INSERT INTO rules(username, id, send_receive_seconds,
						...
						activate, deactivate, is_active) VALUES($sql_username, $id, "
						...
						. (!empty($_GET['query_command_1']) ? pg_escape_literal($_GET['query_command_1'])
						: 'NULL') . ', ' . intval($_GET['send_inject_query_command_nothing'])
						. ', ' . (!empty($_GET['query_command_2']) ? pg_escape_literal($_GET['query_command_2'])
						: 'NULL') . ', ' . (!empty($_GET['proto_id'])
						? '(SELECT proto FROM proto_name WHERE name = ' . pg_escape_literal({$_GET['proto_id']))
						: 'NULL') . ', ' . (!empty($_GET['imm_addr']) ? pgescapebytea($_GET['imm_addr'])
						: 'NULL') . ', ' . pgescapebool($_GET['CCF']) . ', ' . pgescapebool($_GET['ACF'])
						. ', ' . pgescapebool($_GET['broadcast']) . ', ' . pgescapebool($_GET['override_implicit_rules'])
						. 'E, ' . (!empty($_GET['activate']) ? intval($_GET['activate']) : 'NULL')
						. ', ' . (!empty($_GET['deactivate']) ? intval($_GET['deactivate']) : 'NULL')
						. ', ' . pgescapebool($_GET['is_active']) . ");"));
				echo "For username $html_username rule $id inserted.<br/>\n";
			}
			pg_free_result($result);
		} else if (!empty($_GET['key1']) && !empty($_GET['key2'])) {
			$sql_key1 = pg_escape_literal($_GET['key1']);
			$key2 = intval($_GET['key2']);
			$html_key1 = htmlspecialchars($_GET['key1']);
			$result = pgquery("SELECT TRUE FROM users WHERE (username = $sql_key1
					OR username = $sql_username) AND is_administrator;");
			if (($_GET['key1'] == $_SESSION['username'] && $_GET['key1'] == $_GET['username']
				...
				pg_free_result(pgquery("UPDATE rules SET (username, id, send_receive_seconds,
						...
						query_command_1, send_inject_query_command_nothing, query_command_2,
						proto_id, imm_addr, CCF, ACF, broadcast, override_implicit_rules,
						activate, deactivate, is_active) = ($sql_username, $id, "
						. intval($_GET['send_receive_seconds']) . ', '
						. pg_escape_literal($_GET['filter']) . ', '
						. intval($_GET['drop_modify_nothing']) . ', '
						. (!empty($_GET['modification']) ? pg_escape_literal($_GET['modification'])
						...
						. (!empty($_GET['deactivate']) ? intval($_GET['deactivate']) : 'NULL') . ', '
						. pgescapebool($_GET['is_active']) . ") WHERE user = $sql_key1 AND id = $key2;"));
				echo "For username $html_key1 rule $key2 updated.<br/>\n";
			}
			pg_free_result($result);
		}
	} else if (!empty($_GET['key1']) && !empty($_GET['key2'])) {
		$sql_key1 = pg_escape_literal($_GET['key1']);
		$key2 = intval($_GET['key2']);
		$html_key1 = pg_escape_literal($_GET['key2']);
		$url_key1 = urlencode($_GET['key1']);
		$result = pgquery("SELECT TRUE FROM users WHERE username = $sql_key1
		...
		if (($_GET['key1'] == $_SESSION['username'] || $_SESSION['is_administrator']
			...
			if (isset($_GET['confirm'])) {
				pg_free_result(pgquery("DELETE FROM rules WHERE username = $sql_key1
						AND id = $key2;"));
				echo "For username $html_key1 rule $key2 deleted.<br/>\n";
			} else {
?>
				Are you sure?
<?php
				echo "<a href=\"?key1=$url_key1&amp;key2=$key2&amp;delete&amp;confirm\">Yes</a>\n";
?>
				<a href="?">No</a>
<?php
				exit(0);
	...
	if ($_SESSION['is_root']) {
		$result = pgquery('SELECT rules.* FROM rules INNER JOIN users ON rules.username
				= users.username ORDER BY users.is_administrator DESC, rules.username ASC,
				rules.id ASC;');
?>
...
<?php
		...
		$result = pgquery("SELECT rules.* FROM rules INNER JOIN users ON rules.username
				= users.username WHERE rules.username = {$_SESSION['sql_username']}
				OR NOT users.is_administrator ORDER BY users.is_administrator DESC,
				rules.username ASC, rules.id ASC;");
		echo "Viewing table &quot;rules&quot; for username $html_username and
		...
		$result = pgquery("SELECT rules.username, rules.id, rules.send_receive_seconds,
				...
				rules.override_implicit_rules, rules.activate, rules.deactivate,
				rules.is_active, rules.last_run FROM rules INNER JOIN proto_name
				ON rules.proto_id = proto_name.proto
				WHERE rules.username = $_SESSION['sql_username']} ORDER BY rules.id ASC;");
		echo "Viewing table &quot;rules&quot; for username $html_username.<br/>\n";
	}
?>
	<table border="1">
		<tbody>
...
<?php
						...
						echo "<input type=\"text\" value=\"$html_username\" size=\"10\" disabled=\"disabled\"/>\n";
						echo "<input form=\"insert\" type=\"hidden\" name=\"username\" value=\"$html_username\"/>\n";
					}
?>
					,
				</td>
...
<?php
						...
						$form = "form=\"update_{$username}_{$row[1]}\"";
						if ($_SESSION['is_administrator']) {
